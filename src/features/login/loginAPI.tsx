...
export const loginApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    // ✅ NEW: Admin reset withdrawal PIN (set to empty)
    adminResetWithdrawalPin: builder.mutation<
      { message: string },
      { user_id: number }
    >({
      query: ({ user_id }) => ({
        url: "withdrawals/admin/reset-pin",
        method: "PUT",
        body: { user_id },
      }),
    }),
...
export const {
  ...
  useAdminUpdatePasswordMutation,
  useAdminSuspendUserMutation,
  useAdminResetWithdrawalPinMutation, // 👈 NEW EXPORT
} = loginApi;
