  ...
  return (
    ...
    >
      ...
      >
          ...
          {paymentMethod === "mpesa" ? (
            <div>
              ...
              <ol className="list-decimal pl-5 mt-1">
                ...
                <li>
                  Phone Number: <span className="text-lg font-bold">{mpesaNumber}</span>
                </li>
                <li>Name: <span className="text-lg font-bold">{mpesaName}</span></li>
                <li>Amount: <strong>KES {form.amount || "X"}</strong></li>
                <li>Enter your M-Pesa PIN.</li>
                <li>Copy the confirmation message and paste it below.</li>
              </ol>
              <div className="mt-2 flex items-center gap-2">
                <span className="text-xl font-bold">{mpesaNumber} {mpesaName}</span>
                <button
                  type="button"
            ...
            <div>
              ...
              <ol className="list-decimal pl-5 mt-1">
                ...
                <li>Select <strong>Send Money</strong>.</li>
                <li>
                  Phone Number: <span className="text-lg font-bold">{airtelNumber}</span>
                </li>
                <li>Name: <span className="text-lg font-bold">{airtelName}</span></li>
                <li>Amount: <strong>KES {form.amount || "X"}</strong></li>
                <li>Enter your Airtel Money PIN.</li>
                <li>Copy the confirmation message and paste it below.</li>
              </ol>
              <div className="mt-2 flex items-center gap-2">
                <span className="text-xl font-bold">{airtelNumber} {airtelName}</span>
                <button
                  type="button"
                  onClick={() => copyToClipboard(airtelNumber)}
                  className="p-1 bg-white/20 rounded-lg hover:bg-white/30 transition"
                >
                  <ClipboardIcon className="h-5 w-5" />
                </button>
        ...
        <div className="space-y-3">
          <input
            type="number"
            name="amount"
            placeholder="Amount (KES, min 1500)"
            value={form.amount}
            onChange={handleChange}
            className={`w-full bg-white/15 ${
              colors.border
      ...
      >
        <div className="flex items-center justify-between mb-3">
          ...
          <button
            ...
            className={`text-sm flex items-center gap-1 ${colors.text.replace(
              "text-",
              "text-"
            )}300 hover:${colors.text.replace("text-", "hover:text-")}400 transition`}
          >
            <ArrowPathIcon className="h-4 w-4" /> Refresh
          </button>
        </div>
        ...
        {isLoading ? (
          <p className={`${colors.text.replace("text-", "text-")}300 text-center`}>
            Loading deposits...
          </p>
        ) : deposits && deposits.length > 0 ? (
          <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2">
            {deposits.map((deposit) => (
              <div
                key={deposit.id}
                className={`bg-white/15 ${colors.border} rounded-xl p-3 text-sm ${colors.text.replace(
                  "text-",
                  "text-"
                )}100 shadow-sm hover:bg-white/20 transition`}
              >
