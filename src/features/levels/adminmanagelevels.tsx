...
const AdminManageLevels: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payloadData = {
        name: form.name,
        work_deposit: form.work_deposit ? Number(form.work_deposit) : 0,
        number_of_tasks: form.number_of_tasks
          ? Number(form.number_of_tasks)
          : 0,
        mission_income: form.mission_income ? Number(form.mission_income) : 0,
        mission_day_income: form.mission_day_income
          ? Number(form.mission_day_income)
          : 0,
        task_monthly_income: form.task_monthly_income
          ? Number(form.task_monthly_income)
          : 0,
        mission_annual_income: form.mission_annual_income
          ? Number(form.mission_annual_income)
          : 0,
      };
      if (isEditing && form.id != null) {
        await updateLevel({
          level_id: Number(form.id),
          data: payloadData,
        }).unwrap();
        showNotification("Level updated successfully!", "success");
      } else {
  ...
  const sortedLevels = levels ? [...levels].sort((a, b) => a.id - b.id) : [];
      ...
      <div className="max-w-6xl mx-auto overflow-x-auto">
        <table className="w-full text-left border-collapse">
          ...
          <tbody>
            {isLoading ? (
              <tr>
                <td colSpan={9} className="text-center py-4 text-indigo-200">
                  Loading levels...
                </td>
              </tr>
