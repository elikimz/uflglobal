...
//                         M-Pesa Number
//                       </p>
//                       <p className="text-yellow-900">{mpesaNumber}</p>
//                     </div>
//                   )}
//                   {bankName && (
//                     <div className="bg-yellow-50 p-3 rounded-lg">
//                       <p className="text-yellow-700 font-medium">Bank Name</p>
//                       <p className="text-yellow-900">{bankName}</p>
//                     </div>
//                   )}
//                   {bankAccount && (
//                     <div className="bg-yellow-50 p-3 rounded-lg">
...
//                         Bank Account Number
//                       </p>
//                       <p className="text-yellow-900">{bankAccount}</p>
//                     </div>
//                   )}
//                   {fullName && (
//                     <div className="bg-yellow-50 p-3 rounded-lg">
...
//           <p className="text-yellow-700 mb-2">No payment details found.</p>
//           <button
//             onClick={() => setIsEditing(true)}
//             className="bg-yellow-600 hover:bg-yellow-700 transition-colors py-2 px-4 rounded-lg font-semibold text-white"
//           >
//             Add Payment Details
//           </button>
//         </div>
...
const PaymentDetails: React.FC = () => {
  ...
  const handleUpdatePaymentDetails = async () => {
    // Validate M-Pesa number format (Kenyan format: 2547XXXXXXXX)
    if (mpesaNumber && !/^2547\d{8}$/.test(mpesaNumber)) {
      setPaymentFeedback({
        type: 'error',
        message:
          'Please enter a valid Kenyan M-Pesa number (e.g., 254712345678)',
      });
      return;
    }
  ...
  // Handle the "Add Payment Details" button click
  ...
  if (isFetching) {
    return (
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        className="max-w-3xl mx-auto bg-cyan-900/70 backdrop-blur-sm border border-cyan-700 p-6 rounded-2xl shadow-lg"
      >
        <p className="text-cyan-200">Loading payment details...</p>
      </motion.div>
    );
  }
    ...
    >
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-semibold text-cyan-300 flex items-center gap-2">
          <BanknotesIcon className="h-6 w-6 text-cyan-400" /> Payment Details
        </h2>
        {!isEditing && paymentDetails?.has_payment_details && (
          <button
            onClick={() => {
              setIsEditing(true);
              setPaymentFeedback(null);
      ...
      {paymentDetails?.has_payment_details ? (
        <div className="space-y-4">
          <div className="bg-cyan-800/50 p-4 rounded-lg shadow-sm">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {isEditing ? (
                <>
                  <div>
                ...
                <>
                  {mpesaNumber && (
                    <div className="bg-cyan-800/30 p-3 rounded-lg">
                      <p className="text-cyan-300 font-medium">M-Pesa Number</p>
                      <p className="text-cyan-200">{mpesaNumber}</p>
                    </div>
                  )}
                  {bankName && (
                    <div className="bg-cyan-800/30 p-3 rounded-lg">
                      <p className="text-cyan-300 font-medium">Bank Name</p>
                      <p className="text-cyan-200">{bankName}</p>
                    </div>
                  )}
                  {bankAccount && (
                    <div className="bg-cyan-800/30 p-3 rounded-lg">
                      ...
                      <p className="text-cyan-200">{bankAccount}</p>
                    </div>
                  )}
                  {fullName && (
                    <div className="bg-cyan-800/30 p-3 rounded-lg">
                      <p className="text-cyan-300 font-medium">Full Name</p>
                      <p className="text-cyan-200">{fullName}</p>
                    </div>
                  )}
                </>
              )}
          ...
          {isEditing && (
            <div className="flex gap-2 mt-4">
              <button
                onClick={handleUpdatePaymentDetails}
                disabled={isUpdating}
                className="flex-1 bg-cyan-600 hover:bg-cyan-700 transition-colors py-2 rounded-lg font-semibold text-white disabled:opacity-70 disabled:cursor-not-allowed"
              >
                {isUpdating ? 'Updating...' : 'Save Changes'}
              </button>
              <button
                ...
                className="flex-1 bg-gray-600 hover:bg-gray-700 transition-colors py-2 rounded-lg font-semibold text-white"
              >
                Cancel
              </button>
            </div>
        ...
        <div className="bg-cyan-800/30 p-4 rounded-lg text-center">
          <p className="text-cyan-300 mb-2">No payment details found.</p>
          <button
            onClick={handleAddPaymentDetails}
            className="bg-cyan-600 hover:bg-cyan-700 transition-colors py-2 px-4 rounded-lg font-semibold text-white"
          >
            Add Payment Details
          </button>
        </div>
