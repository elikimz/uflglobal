...
const PaymentDetails: React.FC = () => {
  ...
  // Log incoming data from backend
  useEffect(() => {
    console.log('Incoming payment details from backend:', paymentDetails);
  }, [paymentDetails]);

  // Initialize form with fetched data
  useEffect(() => {
    if (paymentDetails) {
      console.log('Initializing form with:', paymentDetails);
      setBankName(paymentDetails.bank_name || '');
      setBankAccount(paymentDetails.bank_account_number || '');
      setFullName(paymentDetails.full_name || '');
      setMpesaNumber(paymentDetails.mpesa_number || '');
  ...
  const handleUpdatePaymentDetails = async () => {
    ...
    try {
      console.log('Updating payment details with:', {
        bank_name: bankName,
